...
def test_import_leaves_out_the_runner() -> None:
    """A run that transfers nothing never reaches rclone, so it should not load it."""
    check = "import backup, sys; assert 'backup.syncer.cli_runner' not in sys.modules"
    result = subprocess.run([sys.executable, "-c", check], check=False)  # noqa: S603
